#!/usr/bin/env bash
# helixllm_describe_challenge.sh — round-295 anti-bluff
...
#
#   normal:    exits 0 only when the runner exits 0 (all 20+
#              invariants pass). Any deviation FAILS.
#
#   mutate:    sets HELIXLLM_MUTATE_RUNNER=1 which inverts
#              invariant 4 inside the runner. The runner MUST
#              then exit non-zero; this wrapper rewrites that
#              non-zero exit to 99 (paired-mutation success).
#              If the runner exits 0 under mutation, this wrapper
#              FAILS — proving the runner actually checks what it
#              claims to check, not a metadata-only PASS.
#
...
#    Challenges MUST guarantee the quality, the completition and
...
MODE="${1:-normal}"
echo "=== HelixLLM Describe Challenge (round-295) ==="
echo "  mode=${MODE}"
...
case "${MODE}" in
    normal)
        ...
        # buried FAILs (no such path today, kept as guard).
        ...
        ;;
    mutate)
        export HELIXLLM_MUTATE_RUNNER=1
        out="$(go run ./challenges/runner/ 2>&1)"
        rc=$?
        echo "${out}" | tail -15
        if [[ "${rc}" -eq 0 ]]; then
