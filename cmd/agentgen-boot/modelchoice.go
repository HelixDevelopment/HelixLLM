...
// file is the FOURTH near-copy of the same decision, after
// cmd/visiongen-boot/modelchoice.go, cmd/imagegen-boot/modelchoice.go and
// cmd/videogen-boot/modelchoice.go. Everything from catalogueDir through
// refusalError is byte-identical across all four but for the lane's forbid-key
// and exit-code names. It belongs in one shared internal package that each lane
// parameterises. That extraction is not made here for a reason worth stating:
// it would rewrite the decision path of three lanes that are currently correct,
// in a change whose subject is a fourth lane that is not, and the cost of a
// mistake in it is every lane at once. The count is recorded so the case for
// extracting it keeps growing visibly rather than quietly.
//
