...
// cmd/agentgen-boot/compose.agent.yml used to carry the same defaulted shape
// with that lane unmigrated, and this note reported the gap rather than sweeping
// it in. That lane has since been migrated and its compose default removed, so
// the gap is closed by cmd/agentgen-boot/compose_model_default_test.go — this
// test's own copy of these three assertions, applied to that file.
